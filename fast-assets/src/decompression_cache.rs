...
impl Drop for CachedFile {
    fn drop(&mut self) {
        match &self.cache {
            Some(cache) => {
                match std::fs::remove_file(cache) {_ => (),};
            }
            None => (),
        }
    }
...
impl CachedFile {
    pub fn set_cache_if_missing(&mut self, cache: &str) {
        match &self.cache {
            ...
            None => {
                ...
                while path.exists() {
                    path.set_file_name(&format!("_{}", path.file_name().unwrap().to_string_lossy().to_string()));
                }
    ...
    pub fn cache(&mut self) {
        match &self.cache {
            Some(path) => {
                let mut file = std::fs::File::options().create(true).write(true).truncate(true).open(path).unwrap();
...
impl DecompressionCache {
    ...
    pub fn load_archive(&mut self, archive: &str, selection: Option<Vec<&str>>) {
        ...
        match path.extension().unwrap().to_string_lossy().to_string().as_str() {
                        ...
                        match selection {
                            Some(ref selection) => {
                                for selected in selection.iter() {
                                    let file_name = file.enclosed_name().unwrap().to_string_lossy().to_string();
            ...
            _ => unimplemented!("Compression format is not supported"),
        }
    }
}
